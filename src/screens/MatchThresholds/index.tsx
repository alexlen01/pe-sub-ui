...
export default function MatchThresholds() {
  ...
  const [thresholds,    setThresholds]    = useState<MatchingThresholds | null>(null)
  const [suffixes,      setSuffixes]      = useState<LegalSuffix[] | null>(null)
  const [abbreviations, setAbbreviations] = useState<KnownAbbreviation[] | null>(null)
  ...
  useEffect(() => {
    ...
    api.config.matching()
      .then(cfg => {
        if (!cfg.thresholds || !Array.isArray(cfg.legalSuffixes) || !Array.isArray(cfg.knownAbbreviations)) {
          throw new Error('Missing matching config sections')
        }
  ...
  const handleSave = useCallback(async (section: string) => {
    if (!thresholds || !suffixes || !abbreviations || loadError) {
      toast('Matching configuration was not loaded; save is disabled until the DB config is available.')
      return
    }
    ...
    setSaving(section)
    try {
      await api.config.setMatching(config, section)
  ...
  }, [thresholds, suffixes, abbreviations, loadError, toast])
  ...
  const set = (field: keyof MatchingThresholds) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setThresholds(prev => prev ? { ...prev, [field]: val } : prev)
  }
  ...
  if (loadError || !thresholds || !suffixes || !abbreviations) {
    return (
      <div style={{ padding: '20px 24px 40px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ padding: '10px 14px', background: '#fff0f0', color: 'var(--danger)', borderRadius: 6, fontSize: 12 }}>
          API error — {loadError ?? 'Matching configuration is incomplete'}. Matching saves are disabled so UI defaults cannot replace database-backed settings.
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: '20px 24px 40px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <Card title="Algorithm Weights" subtitle="Balance between Jaro-Winkler and Levenshtein" action={<Button size="sm" onClick={() => handleSave('weights')} disabled={busy}>{saving === 'weights' ? 'Saving…' : 'Save'}</Button>}>
          <div style={{ padding: '4px 18px 16px' }}>
        ...
        <Card title="Legal Entity Suffix Rules" subtitle="Suffixes stripped before comparison when suffix stripping is enabled" action={<Button size="sm" onClick={() => handleSave('suffixes')} disabled={busy}>{saving === 'suffixes' ? 'Saving…' : 'Save'}</Button>}>
                  ...
                  <tr key={i}>
                    ...
                    <td style={{ padding: '5px 10px', borderBottom: '1px solid var(--border)', textAlign: 'center' }}>
                      <input
                        type="checkbox"
                        checked={s.strip}
                        onChange={() => setSuffixes(prev => prev ? prev.map((r, j) => j === i ? { ...r, strip: !r.strip } : r) : prev)}
                      />
                    </td>
                  </tr>
                ))}
        ...
        <Card title="Abbreviation Expansion Dictionary" subtitle="Known tokens expanded before matching" action={<Button size="sm" onClick={() => handleSave('abbreviations')} disabled={busy}>{saving === 'abbreviations' ? 'Saving…' : 'Save'}</Button>}>
          <div style={{ padding: '0 18px 16px' }}>
            <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--tbl)' }}>
                  ...
                  <tr key={i}>
                    <td style={{ padding: '5px 10px', borderBottom: '1px solid var(--border)', width: 110 }}>
                      <input
                        ...
                        onChange={e => setAbbreviations(prev => prev ? prev.map((r, j) => j === i ? { ...r, token: e.target.value } : r) : prev)}
                    ...
                    <td style={{ padding: '5px 10px', borderBottom: '1px solid var(--border)' }}>
                      <input
                        ...
                        onChange={e => setAbbreviations(prev => prev ? prev.map((r, j) => j === i ? { ...r, expansion: e.target.value } : r) : prev)}
                    ...
                    <td style={{ padding: '5px 10px', borderBottom: '1px solid var(--border)', width: 32, textAlign: 'center' }}>
                      <button
                        onClick={() => setAbbreviations(prev => prev ? prev.filter((_, j) => j !== i) : prev)}
            ...
            <Button
              ...
              size="sm"
              style={{ marginTop: 10 }}
              onClick={() => setAbbreviations(prev => prev ? [...prev, { token: '', expansion: '' }] : prev)}
            >
              + Add Entry
            </Button>
          </div>
